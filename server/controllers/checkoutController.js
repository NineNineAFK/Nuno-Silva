...
const checkoutSuccess = async (req, res) => {
  ...
  try {
    ...
    if (existingOrder) {
      ...
      //     link: `${process.env.URL}/download/${product.productId}`,
      //   })),
      //   licenseKeys: existingOrder.licenseKeys,
      // });
      return res.redirect(`${process.env.CLIENT_URL}/checkout/success?session_id=${existingOrder.checkoutSessionId}`)
    }
    ...
    if (session.payment_status === 'paid') {
      ...
      // Changed: Updated logic to generate license keys for each product quantity
      const licenseKeys = [];
      for (const product of purchasedProducts) {
        for (let i = 0; i < product.quantity; i++) { // Highlighted: Loop based on quantity
          let mask;
          if (product.productName === 'Basic Utility') {
            mask = 'BASIC-****-*****-****-****';
          } else if (product.productName === 'Premium utility') {
          ...
          const keyAuthResponse = await axios.get('https://keyauth.win/api/seller/', {
            params: {
              sellerkey: '411b0232d4e06e2bad91b0ed90fd2c3d',
              type: 'add',
              format: 'json',
              expiry: 10,
              mask,
          ...
          licenseKeys.push({ productName: product.productName, licenseKey: keyAuthResponse.data.key });
          // Highlighted: Add each generated key to the licenseKeys array
        }
      }
      ...
      // res.status(200).json({ downloadLinks, licenseKeys });

      return res.redirect(`${process.env.CLIENT_URL}/checkout/success?session_id=${order.checkoutSessionId}`)
